...
from siriObjects.localsearchObjects import MapItem, ShowMapPoints, MapItemSnippet

class basicDirections(Plugin):
    @register("en-US", "(How|What|Give|Show|Get).*(directions|get) (to|for) (?P<location>[\w ]+?)$")
    @register("en-GB", "(How|What|Give|Show|Get).*(directions|get to)* (?P<location>[\w ]+?)$")
...
class Traffic(Plugin):
    ...
    @register("en-US", ".*traffic like (in|on|near|around|by|at) (?P<location>[\w ]+?)$")
    @register("en-GB", ".*traffic like (in|on|near|around|by|at) (?P<location>[\w ]+?)$")
