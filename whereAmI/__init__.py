...
class whereAmI(Plugin):
    ...
    @register("en-US", ".*(where am i)|(my.*location).*")
        ...
        if jsonString != None:
            ...
            if response['status'] == 'OK':
                ...
                if language == "de-DE":
                    ...
                    self.say("You are currently in {0}.".format(city))
                    the_header+="Here is a map of your location:"
        view = AddViews(self.refId, dialogPhase="Completion")
        mapsnippet = MapItemSnippet(items=[MapItem(label=city+", "+postalCode, street=street, city=city, postalCode=postalCode, latitude=location.latitude, longitude=location.longitude, detailType="CURRENT_LOCATION")])
